...
const GOOGLE_SCRIPT_CODE = `function doGet(e) {
  ...
  if (!withdrawalsSheet) {
    ...
    withdrawalsSheet.appendRow(["Data", "Código", "Código Próprio", "Nome", "Itens Retirados", "Foto"]);
  } else {
    // Ensure header exists if upgrading
    var headers = withdrawalsSheet.getRange(1, 1, 1, withdrawalsSheet.getLastColumn()).getValues()[0];
    if (headers.indexOf("Foto") === -1 && headers.length < 6) {
      withdrawalsSheet.getRange(1, 6).setValue("Foto");
    }
  }
  ...
  if (withdrawalsSheet.getLastRow() > 1) {
    var data = withdrawalsSheet.getRange(2, 1, withdrawalsSheet.getLastRow() - 1, 6).getValues();
  ...
  try {
    ...
    if (data.action === 'addWithdrawal') {
      ...
      if (!sheet) {
        ...
        sheet.appendRow(["Data", "Código", "Código Próprio", "Nome", "Itens Retirados", "Foto"]);
      }
      ...
      if (data.photo) {
        try {
          var folderName = "Fotos_Retiradas";
          ...
          var contentType = data.photo.substring(5, data.photo.indexOf(';'));
          var blob = Utilities.newBlob(Utilities.base64Decode(base64Data), contentType, "foto_" + data.code + ".jpg");
          var file = folder.createFile(blob);
          photoUrl = file.getUrl();
        } catch (e) {
          photoUrl = "Erro ao salvar: " + e.toString();
        }
      }
